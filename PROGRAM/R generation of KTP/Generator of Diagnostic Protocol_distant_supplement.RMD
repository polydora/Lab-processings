...
## `r Year_begin` / `r Year_end` учебный год 



Промежуточной аттестации обучающихся объединения (название сектора, лаборатории) **"Полевой экологии"**

Название дополнительной общеобразовательной программы  - **`r Program_name`**

Фамилия, имя, отчество педагога: **`r teach_groups$Teacher[teach_groups$Group == Group]`** 

Дата проведения: `r itog$Date`

Форма проведения: `r itog$Form_contr`

Форма оценки результатов:    **зачет**

## Результаты  аттестации

## Группа № `r Group`,   год обучения `r Learning_year`


```{r}

contingent <- read_excel("d:/Lab/Lab-processings/Официальные группы/Zajava_generator/Data/Группы ЛЭМБ 2020 2021.xlsx", sheet = "all")

contingent <- contingent[contingent$Group == Group, ]

itog_table <- data.frame(N = 1:nrow(contingent), Name = paste(contingent$Ch_Fam, contingent$Ch_Name),  Result = "Зачет")

# itog_table$Result[sample(1:nrow(contingent), sample(1:2))] <- "Не сдавал"

options(knitr.kable.NA = '')

kable(itog_table, col.names = c("п/п", "Фамилия, Имя обучающегося", "Результат"), align = "llr", row.names = FALSE)

```

\
\
Всего аттестовано ____`r nrow(contingent)`_____ обучающихся. Из них по результатам аттестации:
(Показатель  результативности) ____`r sum(itog_table$Result == "Зачет")` _____чел.
