...
First_day <- "01/09" #в 2019 году для 1 года начало 10 сентября, для всех остальных 2 сентября
Last_day <- "20/05"
...
# название программы #############
Program_name <- "Олимп" #Берется в точности из файла "all_topics.csv"

if(Program_name %in% c("Клуб естествоиспытателей", "Олимп")) holydays <- c(holydays, school_holydays)

# Описание группы ############
Group <- 222

Main_days <- c("Вторник")
# Addition_day <- c("Четверг", "4")
Addition_day <- NA
...
**группа `r Learning_year` года обучения**  
...
all_topics <- read.table("all_topics_short_school_version.csv", header=TRUE, sep=";")
