...
hours <- ktp_print %>% select(Date, Type, Theory, Practice) %>% mutate(Month = factor(months(as.Date(Date)), levels = c("Сентябрь", "Октябрь","Ноябрь", "Декабрь", "Январь","Февраль", "Март","Апрель", "Май", "Июнь"))) %>% melt() %>% group_by(Month, Type) %>% summarise(Sum_hour = sum(value)) %>% dcast(Month ~ Type) %>% mutate(Theory = Main + Virtual)
