...
if(("Excursion" %in% names(hours)) & ("Virtual" %in% names(hours))) hours %>% mutate(Theory = Main + Virtual, Practice = Excursion) %>% mutate(Cum_hours_Practice = cumsum(Practice), Cum_hours_Theory = cumsum(Theory), Rest = sum(c(Excursion, Main, Virtual), na.rm = T) - (Cum_hours_Practice + Cum_hours_Theory), Group = Group) %>% relocate(Group)  %>% write.table(., "clipboard", sep = "\t", row.names = F, fileEncoding = "native.enc")
...
if(("Excursion" %in% names(hours)) & !("Virtual" %in% names(hours))) hours %>% mutate(Theory = Main, Practice = Excursion) %>% mutate(Cum_hours_Practice = cumsum(Practice), Cum_hours_Theory = cumsum(Theory), Rest = sum(c(Excursion, Main), na.rm = T) - (Cum_hours_Practice + Cum_hours_Theory), Group = Group) %>% relocate(Group)  %>% write.table(., "clipboard", sep = "\t", row.names = F, fileEncoding = "native.enc")
